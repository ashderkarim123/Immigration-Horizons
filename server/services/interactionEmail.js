 ...
 *
 * Mirrors src/lib/auth/email.ts's transport usage and "missing key/failed
 ...
 *
 * Failure-mode tests inject through services/mailer.js's
 * `_setTransportForTests` rather than a per-service hook, so no test here
 * depends on a real mail account (module doc: "Add test doubles or
...
async function sendInteractionEmail({ to, subject, html }) {
  return sendMail({ to, subject, html, tag: 'interaction-email' });
}
