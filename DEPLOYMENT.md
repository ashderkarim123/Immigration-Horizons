...
| `SITE_URL` | **Yes** | **Not the same variable as `NEXT_PUBLIC_SITE_URL`, and not the same value as `server/.env`'s `SITE_URL`.** Set it to `https://app.immigrationhorizons.com` — the host the portal and staff console are served from since ADR-008. Two things depend on it: (1) the CSRF `Origin` check in `src/lib/auth/csrf.ts`, which every mutating `/api/portal/*` and `/api/staff/*` route runs — a wrong value makes **every** client and staff write fail with 403; (2) the activation and password-reset links in `src/lib/auth/email.ts`, which must point at `/portal/activate` on the app host. Unset, it falls back to `http://localhost:3000` and nothing mutating works in production. |
| `APP_TIMEZONE` | Recommended | IANA zone (e.g. `America/New_York`) defining "today" for the scheduled-consultation queues in both apps. Defaults to `UTC`; a fixed-offset abbreviation like `EST` is rejected and falls back to UTC with a warning. Set the same value here and in `server/.env`, or the two apps disagree about which consultations are today. |
